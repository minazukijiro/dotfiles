...
(leaf open-junk-file
 ...
 (leaf org-mode
  ...
  (defun my:org-capture-new-expenses ()
    (let* ((memo (read-from-minibuffer "memo: "))
           (amount (read-from-minibuffer "amount: ")))
      (format "| # | %t | %s | %d | |" memo amount)))
  ...
  (org-capture-templates . '(("t" "Todo" entry
                             ...
                             ("w" "Wish list" checkitem
                              (file (concat org-directory "/wishlist.org"))
                              (function my:org-capture-new-wish-list)
                              :immediate-finish t)
                             ("e" "Expenses" table-line
                              (file (concat org-directory "/expenses.org"))
                              "| # | %^T | | | |")
                             ))
